...
const ArtworkCard: React.FC<ArtworkCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "flex flex-col relative items-start w-full max-w-xs sm:w-40 md:w-70 lg:w-76 aspect-[302/500] bg-gray-100 rounded-lg cursor-pointer hover:shadow-sm transition-all duration-300",
        className,
    ...
    >
      ...
      <div className="flex justify-center w-full items-center pt-3 sm:pt-4 md:pt-5 px-3 sm:px-4 md:px-5 pb-2">
        <div className="w-full h-48 sm:h-56 md:h-64 lg:h-72 bg-gray-200 rounded overflow-hidden">
          <img
            src={imageUrl}
            alt={title}
      ...
      <div className="flex items-center py-1 sm:py-2 px-4 sm:px-5 md:px-6">
        <p className="font-semibold text-sm sm:text-lg md:text-xl lg:text-2xl text-zinc-900 truncate">
          {title}
        </p>
      </div>
      ...
      {author && (
        <div className="flex items-center py-1 sm:py-2 px-4 sm:px-5 md:px-6">
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-zinc-900 truncate">
            {author}
          </p>
        </div>
      )}
